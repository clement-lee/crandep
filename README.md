...
dependencies (Imports) for one package (dplyr):
    ...
    qux
    #> [1] "assertthat (≥ 0.2.0), glue (≥ 1.1.1), magrittr (≥ 1.5), methods, pkgconfig (≥ 2.0.1), R6 (≥ 2.2.2), Rcpp (≥ 1.0.0), rlang (≥ 0.3.0), tibble (≥ 2.0.0), tidyselect (≥ 0.2.5), utils"

    ## Split the above string into a vector
    quz <- get_dep_vec(qux)
    quz
    #>  [1] "assertthat" "glue"       "magrittr"   "methods"    "pkgconfig" 
    #>  [6] "R6"         "Rcpp"       "rlang"      "tibble"     "tidyselect"
    #> [11] "utils"
